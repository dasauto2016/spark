...
object TDWRDDTest {
  def main(args: Array[String]) {
    ...
    val db = cmdArgs.getStringValue("db")
    val srcTbl = cmdArgs.getStringValue("src_tbl")
    val priParts = cmdArgs.getCommaSplitArrayValue("pri_parts", null)
    val subParts = cmdArgs.getCommaSplitArrayValue("sub_parts", null)

    val tdwurl = cmdArgs.getStringValue("tdwurl")

    // val tdw = new TDWProvider(sc, "test_user", "test_passwd", "test_db")
    val tdw = new TDWProvider(sc, db)
...
//    tdw.saveToTable(rdd, tblName, priPartName, subPartName)
    ...
    rdd.saveToTable(tdwurl, overwrite = true)
  }
}
